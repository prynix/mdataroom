<table cellpadding="0" cellspacing="0" border="0" class="display groceryCrudTable" id="<?php echo uniqid(); ?>">
	...
	<tbody>
		...
		<tr id='row-<?php echo $num_row?>'>
			<?php foreach($columns as $column){?>
				<td class="<?php echo $column->field_name?>"><?php echo $row->{$column->field_name}?></td>
			<?php }?>
			<?php if(!$unset_delete || !$unset_edit || !$unset_read || !empty($actions)){?>
			<td class='actions'>
				<?php
	...
	<tfoot>
		<tr>
			<?php foreach($columns as $column){?>
                                <?php
                                    $countList = array('request', 'impression', 'click', 'CTR', 'Display_CPM', 'Display_CPC',  'Ad_Server_CPM', 'Ad_Server_CPC', 'Display_Budget', 'Ad_Server_Cost', 'Total_Cost');
                                    //$column->field_name === 'click' ||$column->field_name === 'impression' || $column->field_name === 'request' || $column->field_name === 'CTR' || $column->field_name === 'Display_CPM' 
                                    if (in_array($column->field_name, $countList)){
                                ?>
                                <th><span id="<?php echo $column->field_name;?>"></span></th>
                                <?php    } else {
                                ?>
