...
//astarserve_namesp.load_process_url = "http://mdataroom.com/adserver/as/index.php/ServiceController2/process_load_request";
astarserve_namesp.load_process_url = "http://localhost/mdataroom.com/index.php/ServiceController2/process_load_request";
//
//astarserve_namesp.load_process_url = "http://localhost/astarserve/as/index.php/ServiceController2/process_load_request";


//astarserve_namesp.impression_process_url = "http://localhost/astarserve/as/index.php/azadtest2/process_impression_request";
//astarserve_namesp.load_process_url = "http://localhost/astarserve/as/index.php/azadtest2/process_load_request";
...
{
    //if(astarserve_namesp.sentAlready===true) return;
    
    //astarserve_namesp.sentAlready=true;
...
//    sdata["uuid"] = astarserve_namesp.getUUID();
    ...
    var banner_id = astarserve_namesp.getBannedId(elem.id);
    var uuid = astarserve_namesp.getUUID();
    //send data back to parent window
//     window.parent.postMessage({message: elem.id.'_'.banner_id}, "*");
    window.parent.postMessage({pid: elem.id, bid:banner_id, uuid:uuid }, "*");
};

astarserve_namesp.getBannedId = function(placementId)
{
    var placements=$.cookie('placements');
    var bannars= $.cookie('banners');
    //alert(placements);
    var mdpId=placementId.slice(2);
    var placementlist=placements.split("+");
    var bannerlist=bannars.split("+");
    
    for(var i=0;i<placementlist.length;i++)
    {
        if(placementlist[i]==mdpId)return bannerlist[i];
    }
